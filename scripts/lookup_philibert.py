...
def lookup_one(session, survivor: dict, rate_limit_sec: float, override_title: str | None = None) -> dict:
    """Returns a Philibert-status dict: {status, price_eur, language, url}.

    `override_title`, when given, is a human-confirmed real French title from
    data/philibert_title_overrides.json -- for the class of miss where the French edition's
    title is a genuine translation rather than a variant of the English one (e.g. "EXIT: The
    Venice Conspiracy" is sold in France as "EXIT - Intrigue à Venise"; the shared words are
    "EXIT" and a place name that isn't even spelled the same, "Venice"/"Venise" -- nothing our
    automatic translation/prefix/fuzzy tiers can bridge). Tried right after EAN, ahead of the
    ordinary title search, since it's a confirmed answer rather than a guess.
    """
    ...
    if not url and override_title:
        url = search_by_title(session, override_title)
        time.sleep(rate_limit_sec)
...
def _load_title_overrides(path: str) -> dict[str, str]:
    """data/philibert_title_overrides.json: zatu_handle -> a human-confirmed real French title,
    for misses no automatic heuristic can bridge (a genuine translation, not a variant title).
    Manually maintained, never auto-regenerated by any pipeline stage -- same pattern as
    data/excluded_games.json and data/bgg_fr_editions.json."""
    file = Path(path)
    if not file.exists():
        return {}
    try:
...
def main() -> int:
    ...
    parser.add_argument("--title-overrides", default="data/philibert_title_overrides.json")
    ...
    title_overrides = _load_title_overrides(args.title_overrides)
    print(
        ...
        f"{len(title_overrides)} manual title overrides available)...",
        file=sys.stderr,
    )
    ...
    for i, survivor in enumerate(survivors, start=1):
        ...
        if cached and (args.offline or cached.get("philibert_status") in _CACHEABLE_STATUSES):
            ...
            try:
                override_title = title_overrides.get(survivor["zatu_handle"])
                philibert = lookup_one(session, survivor, rate_limit, override_title=override_title)
